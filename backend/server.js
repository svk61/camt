...
app.post('/api/assessment/submit', authMiddleware, async (req, res) => {
  try {
    ...
    // Simple channel assignment logic based on answers
    const assignedChannels = ['general']; // Everyone gets general
    
    // Add specific channels based on answers
    if (answers.q2 === 'Career advice') {
      assignedChannels.push('career-advice');
    }
    if (answers.q2 === 'Mentorship') {
      assignedChannels.push('mentorship');
    }
    if (answers.q2 === 'Networking') {
      assignedChannels.push('networking');
    }
    if (answers.q2 === 'Emotional support') {
      assignedChannels.push('support');
    }
    
    // Update user
    req.user.assessmentAnswers = answers;
    req.user.hasCompletedAssessment = true;
    req.user.assignedChannels = assignedChannels;
    await req.user.save();
    
    res.json({ assignedChannels });
  } catch (error) {
